    ...
    if (Token::Verify($token, KEY)) {
        switch ($method) {
            ...
            case 'POST':
                ...
                if ($stmt_validation1->rowCount() !== 0) {
                    ...
                    if ($stmt_validation->rowCount() == 0) {
                            ...
                            $stmt_post->bindParam(':IdModulo', $Nombre);
                            $stmt_post->bindParam(':Nombre', $Nombre);
                            $stmt_post->bindParam(':OrdenMenu', $Nombre);
            ...
            case 'PUT':
                ...
                if ($stmt_validation->rowCount() == 0) {
                    $query_validation = "Select Nombre From MODULO Where IdModulo=:IdModulo";
                    $stmt_validation = $dbhost->prepare($query_validation);
                    $stmt_validation->bindParam(':IdModulo', $IdModulo);
                    $stmt_validation->execute();

                if ($stmt_validation->rowCount() !== 0) {
                        $query_post = "UPDATE MENU SET IdModulo=:IdModulo, Nombre=:Nombre, OrdenMenu=:OrdenMenu,
                        FechaModificacion=:FechaHoraActual,
                        UsuarioModificacion=:UsuarioModificacion
                        WHERE IdMenu=:idMenu";
                        $stmt_post = $dbhost->prepare($query_post);
                        $stmt_post->bindParam(':idModulo', $IdGenero);
                        $stmt_post->bindParam(':Nombre', $IdGenero);
                        $stmt_post->bindParam(':OrdenMenu', $Nombre);
                        $stmt_post->bindParam(':FechaHoraActual', $fechaHoraActual);
            ...
            case 'DELETE':
                ...
                $IdMenu = $data["idMenu"];
                $query_delete = "DELETE FROM GENERO WHERE IdMenu=:IdMenu";
                $stmt_post = $dbhost->prepare($query_delete);
                $stmt_post->bindParam(':idMenu', $IdGenero, PDO::PARAM_INT);
                $stmt_post->execute();
