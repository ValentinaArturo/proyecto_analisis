...
if ($token) {
    ...
    if (Token::Verify($token, KEY)) {
        switch ($method) {
            case 'GET':
                $query_validation = "SELECT * FROM GENERO";
            ...
            case 'POST':
                ...
                $query_validation = "Select Nombre From GENERO Where Nombre=:Nombre";
                $stmt_validation = $dbhost->prepare($query_validation);
                $stmt_validation->bindParam(':Nombre', $Nombre);
                $stmt_validation->execute();

                if ($stmt_validation->rowCount() == 0) {
                    $query_post = "INSERT INTO GENERO(Nombre,FechaCreacion,UsuarioCreacion,FechaModificacion,UsuarioModificacion) 
                    ...
                    if ($stmt_post->rowCount() > 0) {
                        echo json_encode(
                            array(
                                "status" => 200,
                                "msg" => "Genero agregado exitosamente"
                            )
                        );
                    ...
                    echo json_encode(
                        array(
                            "status" => 401,
                            "msg" => "¡El nombre del genero está en uso, valide!"
                        )
                    );
                    break;
                }
            ...
            case 'PUT':
                ...
                if ($stmt_validation->rowCount() == 0) {
                    $query_post = "UPDATE GENERO SET Nombre=:Nombre,
            ...
            case 'DELETE':
                ...
                $IdGenero = $data["idGenero"];
                $query_delete = "DELETE FROM GENERO WHERE IdGenero=:idGenero";
                $stmt_post = $dbhost->prepare($query_delete);
                $stmt_post->bindParam(':idGenero', $IdGenero, PDO::PARAM_INT);
                $stmt_post->execute();
